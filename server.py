...
layout = [
    [sg.Text('Port Number: '), sg.InputText(size=(30, 2), key='-PORT-'),
     sg.Button('Start Listening', key='-LISTEN-', disabled=False)],
    [sg.Text(key='-ERROR-')],
...
# This thread is responsible for retrieving data from a client and passing it down to other connected clients.
def new_client(clientSocket, clientAddress):
    while True:
        ...
        # if disconnected message received from the client, then close that socket.
        if clientData == 'Disconnected':
            ...
            messages.append(message)
            window['-MSG_BOX-'].update('\n'.join(messages))
            # all other clients except the sending client are being sent the message
...
while True:
    ...
    if event == '-SERVERINPUT-':
        if values['-SERVERINPUT-'] == '' or values['-SERVERINPUT-'] == ' ' or values['-SERVERINPUT-'] is None:
