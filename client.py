...
layout = [
    [sg.Text('IP-Address: '), sg.InputText(size=(15, 2), key='-IPADDRESS-', enable_events=True),
     sg.Text('Connect to Port: '), sg.InputText(size=(15, 2), key='-PORT-', enable_events=True),
...
# This client thread receives messages from the server
def client_function():
    while True:
        try:
            messageFromServer = client.recv(2048).decode('utf-8')
            if not messageFromServer:
                raise ConnectionResetError  # if connection is closed this event is raised
...
while True:
    ...
    if event == '-SEND-':  # handles client sending message to the server
        try:
            ...
            window['-ERROR-'].update(e)
    if event == '-DISC-':  # handles when client disconnected from the server
        mess = 'Disconnected'
